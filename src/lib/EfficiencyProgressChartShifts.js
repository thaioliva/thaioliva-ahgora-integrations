...
const EfficiencyProgressChartShifts = (props) => {
  ...
  const isDay = props.match.params.viewType === 'day';
  const isMonth = props.match.params.viewType === 'month';
  const isWeek = props.match.params.viewType === 'week';
      ...
      return (
        <text x={props.x} y={props.y}>
          <tspan x={props.x} dy="15" textAnchor="middle" fill="#666">
            {moment(props.payload.value).format('HH')}h
          </tspan>
        </text>
      );
    }
  ...
  const CustomizedTooltip = (props) => {
    ...
    // if (value.countTotal === null) return null;
      ...
      <div className="chart-tooltip">
        <div className="chart-tooltip-title">
          <h3 className="title is-6">
            {(isDay) ?
              moment(value.timestamp).format('HH') + 'h' :
      ...
      <ul style={{ textAlign: 'center' }} className="chart-legend">
        {legendKeys.map((entry, index) => (
          <li key={`item-${index}`} className="chart-legend-item">
            <span className="chart-legend-icon" style={{ backgroundColor: entry.color || colors[index] }}></span>
            {entry.label}
          </li>
        ))}
      </ul>
  ...
  if (!isCustomRange)
    legendKeys.push({ label: 'Production Goal', color: '#55CF9C' });
  ...
  return (
    <div className="axis-chart">
      ...
      {/* {<InfoBoxChart data={data} />} */}
    </div>
  );
};

EfficiencyProgressChartShifts.propTypes = {};

export default EfficiencyProgressChartShifts;